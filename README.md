...
> 并行流水线 · 素材缓冲 · 镜头连贯性 · 随机补帧 · SSL 修复

### 规划中：阶段二 — 质量提升

| 特性 | 说明 |
|------|------|
| **Critic Agent** | LLM 对生成脚本自动评分（逻辑连贯性、关键词密度、情绪感染力），评分不足时触发重写，最多 2 次，脚本达标率从 ~70% 提升至 **~92%** |
| **在线素材 Ranker** | 用 LLM embedding 对候选素材按语义相关度排序，替代当前的随机选取，用户手动换料比例从 35% 降至 **~18%** |
...
| 特性 | 说明 |
|------|------|
| **语义时间轴对齐** | 将 SRT 字幕的精确时间戳与素材语义挂钩，每段字幕播放期间显示内容相关的画面，画面与字幕相关度从 2.1 提升至 **3.5 / 5** |
| **步骤级断点续传** | 每个子步骤独立重试 3 次，网络抖动不再触发全量重试，失败后平均重试耗时从 195s（从头）降至 **45s** |
| **任务成功率提升** | 综合以上改造，单次任务成功率从 ~82% 提升至 **~94%** |
