...
> 并行流水线 · 素材缓冲 · 镜头连贯性 · 随机补帧 · SSL 修复 · **Critic Agent** · **素材智能排序（Ranker）** · **语义时间轴对齐** · **步骤级断点续传** · **稳定性回归测试套件** · **结构化任务日志** · **本地记忆基础设施（M1）**

### 进行中：用户体验记忆模块（M2 + M3）

| 阶段 | 特性 | 说明 |
|------|------|------|
| **M2** | **跨任务素材去重 / 复用** | 基于 M1 SQLite 记忆，按"最近用过则降权"的策略改写候选排序：新鲜素材按 LLM 相关度优先，最近 7/30 天用过的素材自动后置；目标把同 URL 在最近 5 个任务中的重复出现率从 ~25% 压到 <8% |
| **M3** | **脚本风格学习 + 失败模式记忆** | 用户在 WebUI 接受/拒绝/手改的脚本样本作为 Critic Agent 输入，长期对齐用户表达偏好；失败任务的阶段+错误自动入库，retry 时显示"上次同类失败的解决方式"；附带 WebUI 设置面板，所有记忆数据可逐条查看与一键清空 |
...
| 回归 bug 复发率 | 高（无回归测试） | **0**（30 用例守门） | 同类 bug 不再回归 |
